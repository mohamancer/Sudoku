...
/****************************************** Module ******************************************
 *  Module: LP Solver
 *  Implements: LP_Solver.h
 *  Includes: gurobi_c.h
 *  Usage:
 *      This modules implements the LP algorithm that solves the board using Gurobi.
 *      It include functions used internally to communicate with the gurobi package 
 *      and to aid in performing the solve_board_with_LP function.
...
static RETURN_STATUS set_board_solution(GRBmodel *, int , int,double * );
...
{
	int error, num_of_variables, N,loop,game_status;
    double *sol;
    GRBenv *env = NULL;
    GRBmodel *model = NULL;
	...
	if(sol == NULL){
        printf("allocation error in solve_board_with_LP\n");
        return ALLOC_FAIL;
    }
    ...
    if(game_status != GRB_OPTIMAL){
        ...
        status = set_board_solution(model, block_rows, block_cols,sol);
        if(status != LP_SUCCESS){
            return status;
        }
    }
	for (loop=0;loop<num_of_variables;loop++){
		arr[loop]=sol[loop];
	}
	free(sol);
...
/*
 *  Function to update array with the scores  
 *  provided that the optimizer managed to find a solution to 
 *  the problem.
 */ 
static RETURN_STATUS set_board_solution(GRBmodel *model, int block_rows, int block_cols, double *arr )
{
    int error, num_of_variables, N, i;
    double *sol;
    N = block_rows*block_cols;
    num_of_variables = N*N*N;
    sol = malloc(sizeof(double)*num_of_variables);
...
/*
 *  Function to get the solution status of the LP
 ...
 *  -   the objective function is maximization with random coefficients.     just need to find a feasible solution.
 */ 
...
static RETURN_STATUS add_variables(GRBenv *env, GRBmodel *model, int N)
{
    ...
    num_of_variables = N*N*N;
	obj = (double *) calloc(num_of_variables, sizeof(double));
	ul = (double *) calloc(num_of_variables, sizeof(double));
    vtype = (char *) calloc(num_of_variables, sizeof(char));
    if(vtype == NULL || obj==NULL || ul==NULL){
        printf("Can't allocate memory for vtype in add_variables\n");
        return ALLOC_FAIL;
    }
    /*Setting variable types to CONTINUOUS*/
    for(i=0; i<num_of_variables; i++){
		obj[i]=(double)(rand()%(N+1));
    ...
    error = GRBaddvars( model,
            ...
            NULL,                    
            NULL,                   /* with variables of CONTINUOUS type with lower limit */
            NULL,                   /* of zero and upper limit of 1 */
            obj,                   /* no names given to variables. */
            NULL,
            ul,
            vtype,
            NULL
